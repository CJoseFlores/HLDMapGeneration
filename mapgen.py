...
def generateFloors(out_file):
    ...
    for i in range(0, 15*2):
        ...
        out_file.write("\n     obj,Scenery,"+str(sceneryID)+","+str(sceneryX)+","+str(sceneryY)+",9,-999999,++,0="+scenerySpr[sprIndex]+",1=1,2=0,3=0,k=0,p=-4,fp=0,4=0,5=0,f=1,l=0,")
...
def spawnLoot(out_file,xPos,yPos):
    ...
    if lootValue <= 3: #Healthkit
        uniqID += 1
        out_file.write("\n     obj,HealthKit,"+str(uniqID)+","+str(xPos)+","+str(yPos)+",3,-999999,++,")
    elif lootValue <= 5: #Gearbit
        uniqID += 1
        out_file.write("\n     obj,Spawner,"+str(uniqID)+","+str(xPos - 15)+","+str(yPos - 15)+",4,-999999,++,-1=GearbitCrate,-2=-999999,-4=1,-5=0,-6=-1,-7=0,-8=0,")
        uniqID += 1
        out_file.write("\n     obj,Spawner,"+str(uniqID)+","+str(xPos - 15)+","+str(yPos + 15)+",4,-999999,++,-1=GearbitCrate,-2=-999999,-4=1,-5=0,-6=-1,-7=0,-8=0,")
        uniqID += 1
        out_file.write("\n     obj,Spawner,"+str(uniqID)+","+str(xPos + 15)+","+str(yPos - 15)+",4,-999999,++,-1=GearbitCrate,-2=-999999,-4=1,-5=0,-6=-1,-7=0,-8=0,")
        ...
        if outfitType == 1:
            out_file.write("\n     obj,DrifterBones_Outfit,"+str(uniqID)+","+str(xPos)+","+str(yPos)+",3,-999999,++,spr=spr_UiSwordIcon,i="+str(outfitLootVal)+",f=0,k=0,g=0,c=0,s="+str(outfitSpriteVal)+",w=-999999,")
        elif outfitType == 2:                    
            out_file.write("\n     obj,DrifterBones_Outfit,"+str(uniqID)+","+str(xPos)+","+str(yPos)+",3,-999999,++,spr=spr_uiCapeIcon,i="+str(outfitLootVal)+",f=0,k=0,g=0,c="+str(outfitSpriteVal)+",s=0,w=-999999,")
        else:
            out_file.write("\n     obj,DrifterBones_Outfit,"+str(uniqID)+","+str(xPos)+","+str(yPos)+",3,-999999,++,spr=spr_ghost,i="+str(outfitLootVal)+",f=0,k=0,g="+str(outfitSpriteVal)+",c=0,s=0,w=-999999,")
    else:
        uniqID += 1
        weaponType = randint(1,5)
        if weaponType == -1: #The pistol cannot be picked up if other weapons are picked up
            out_file.write("\n     obj,DrifterBones_Weapon,"+str(uniqID)+","+str(xPos)+","+str(yPos)+",8,-999999,++,spr=spr_itemsGUI,i=1,f=0,k=0,g=0,c=0,s=0,w=1,")
        elif weaponType == 2:
            out_file.write("\n     obj,DrifterBones_Weapon,"+str(uniqID)+","+str(xPos)+","+str(yPos)+",8,-999999,++,spr=spr_itemsGUI,i=2,f=0,k=0,g=0,c=0,s=0,w=2,")
        elif weaponType == 3:
            out_file.write("\n     obj,DrifterBones_Weapon,"+str(uniqID)+","+str(xPos)+","+str(yPos)+",8,-999999,++,spr=spr_itemsGUI,i=21,f=0,k=0,g=0,c=0,s=0,w=21,")
        elif weaponType == 4:
            out_file.write("\n     obj,DrifterBones_Weapon,"+str(uniqID)+","+str(xPos)+","+str(yPos)+",8,-999999,++,spr=spr_itemsGUI,i=23,f=0,k=0,g=0,c=0,s=0,w=23,")
        elif weaponType == 5:
            out_file.write("\n     obj,DrifterBones_Weapon,"+str(uniqID)+","+str(xPos)+","+str(yPos)+",8,-999999,++,spr=spr_itemsGUI,i=41,f=0,k=0,g=0,c=0,s=0,w=41,")
        else:
            out_file.write("\n     obj,DrifterBones_Weapon,"+str(uniqID)+","+str(xPos)+","+str(yPos)+",8,-999999,++,spr=spr_itemsGUI,i=43,f=0,k=0,g=0,c=0,s=0,w=43,")
...
def spawnMobs(out_file,mobName,xStart,xEnd,yStart,yEnd,count):
    ...
    for i in range(0, count):
        ...
        out_file.write("\n     obj,Spawner,"+str(uniqID)+","+str(mobX)+","+str(mobY)+",4,-999999,++,-1="+mobName+",-2=-999999,-4=1,-5=0,-6=-1,-7=0,-8=0,")
...
def spawnScenery(out_file,sceneryName,xStart,xEnd,yStart,yEnd,count):
    ...
    for i in range(0, count):
        ...
        out_file.write("\n     obj,Scenery,"+str(sceneryID)+","+str(sceneryX)+","+str(sceneryY)+",2,-999999,++,0="+sceneryName+",1=0,2=0,3=0,k=0,p=-4,fp=0,4=0,5=0,f=0,l=0,")
...
def generateLevelContent(out_file,cols,rows,difficulty):
    ...
    yStart = 60
    yEnd = 2000
    xInc = int((xEnd - xStart)/cols)
    yInc = int((yEnd - yStart)/rows)
    ...
    for c in range (1,cols+1):
        ...
        for r in range (1,rows+1):
            ...
            if c <= 2 or spawnValue <= 100-difficulty*7:
                ...
                print("Spawning "+str(mobNumber)+" "+mobName)
...
while True:
    if time.time()-lastTime > 2.0:
        ...
        # Get the timestamp of the last time the center level was loaded by the game.
        currRead = os.stat('central/rm_c_backertabletx.lvl').st_atime

        # Regenerate the west level only when the center level has been loaded again.
        if lastRead != currRead:
            lastRead = currRead
            generation += 1
            difficulty = int(generation/5)
            if difficulty > 10:
                difficulty = 10
